class File
  ...
  def gets(sep_string = $/)
    ...
    sep = /.*?#{Regexp.escape(sep_string)}|.+/
    
    chunk = String.new
    while chunk and chunk !~ sep
    ...
    if chunk
      @line_buffer[line_buffer_pos] = "#{@line_buffer[line_buffer_pos]}#{chunk}"
    else
      # at the end
      return @line_buffer.shift
    end
    ...
    # 
    @line_buffer[line_buffer_pos] = @line_buffer[line_buffer_pos].scan(sep)
    @line_buffer.flatten!
